...
def non_linear_route(t,a,b,c):
    ...
    I = lambda x,s:re_sqa*log(abs(2*a*x+b+2*sqa*s)+1e-35)
    it = I(t,st)
    i0 = I(0,s0)
    it0 = it - i0
    tit = t * it
...
def acc_lim_route(tf,px_,pv0_,x0_,v0_,acc_lim=1.0):
    p = -np.array(px_)
    q = np.array(pv0_)
    x0 = np.array(x0_)
    v0 = np.array(v0_)
    ...
    if (b*b) == (4*a*c):
        ...
        q1,p1,q2,p2 = calc_integration(tf*acc_lim,a,b,c)
        vtf = acc_lim * (q1 * q + p1 * p) + v0
